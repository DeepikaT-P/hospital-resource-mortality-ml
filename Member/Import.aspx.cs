...
namespace medicalProject.Member
{
    public partial class Import : System.Web.UI.Page
    {
        ...
        private void Import_To_Grid(string FilePath, string Extension, string _Location)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                DataTable tabloca = new DataTable();
                tabloca = obj.GetDatasetByHospital(_Location);

                if (tabloca.Rows.Count > 0)
                {
                    obj.DeleteDataByLoc(_Location);
                }
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    obj.InsertDataset(_Location, DateTime.Parse(dt.Rows[i]["Date"].ToString()),
                ...
                ClientScript.RegisterStartupScript(this.GetType(), "Key", "<Script>alert('Data Uploaded Successfully for " + _Location + "!!!')</script>");
            }
            else
            {
