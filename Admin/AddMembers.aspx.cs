    ...
    public partial class AddMembers : System.Web.UI.Page
    {
        ...
        void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                obj.DeleteIA(s[1]);
                ClearTexts();
                ClientScript.RegisterStartupScript(this.GetType(), "key", "<script>alert('Incharger Deleted Successfully!!!')</script>");
                GetHospitals();
            }
            catch
            {
                ...
                if (DropDownListCity.SelectedIndex > 0)
                {
                    if (btnIC.Text == "Add")
                    {
                        if (obj.CheckICId(txtICID.Text))
                        {
                            ...
                            ClientScript.RegisterStartupScript(this.GetType(), "key", "<script>alert('Hospital Added Successfully!!')</script>");
                            ClearTexts();
                            GetHospitals();
                        }
                        else
                        {
                            ClientScript.RegisterStartupScript(this.GetType(), "key", "<script>alert('Incharger Id Exists!!!')</script>");
                        }
                    }
                    else if (btnIC.Text == "Update")
                    {
                        if (txtICID.Text.Equals(Session["IC"].ToString()))
                        {
                            ...
                            ClientScript.RegisterStartupScript(this.GetType(), "key", "<script>alert('Hospital Updated Successfully!!!')</script>");
                            ClearTexts();
                            GetHospitals();
                            btnIC.Text = "Add";
                        }
                        else
                        {
                            ...
                            {
                                obj.UpdateIC(txtICID.Text, txtPassword.Text, txtName.Text, txtAddress.Text, int.Parse(DropDownListCity.SelectedValue), txtContactNo.Text, txtEmailId.Text, Session["IC"].ToString());
                                ClientScript.RegisterStartupScript(this.GetType(), "key", "<script>alert('Incharger Details Updated Successfully!!!')</script>");
                                ClearTexts();
                                GetHospitals();
                                btnIC.Text = "Add";
                            }
